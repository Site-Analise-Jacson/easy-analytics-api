...
import { VirtualSportTeam } from 'src/shared/domain/VirtualSportTeam';
import { VirtualSportGame } from 'src/shared/domain/VirtualSportGame';
import { VirtualSportEvent } from 'src/shared/domain/VirtualSportEvent';
import { Br4BetService } from '../services/br4bet.service';
...
export class Br4BetSchedule {
    ...
    constructor(
        private readonly br4BetService: Br4BetService,
    ...
    async handleCron() {
        try {
            ...
            this.logger.log(`Quantidade de jogos: ${call.length}`);
            ...
            this.logger.log(`Event Gorups Count: ${domainGames.length}`);
            ...
            for (const game of domainGames) {
                ...
                const search = `${game.dbId}_${game.championId}_goldebet_${game.champion.normalize().replaceAll(" ", "_").toLowerCase()}_${game.eventId}_${game.externalId}`
                ...
                const originalDate = new Date(game.startDate);
                ...
                this.websocket.sendBroadcast(
                    'goldebet',
                    game.champion.toLowerCase().replaceAll(" ", "_").replaceAll("-", "_"),
                    payload
                );
            }
