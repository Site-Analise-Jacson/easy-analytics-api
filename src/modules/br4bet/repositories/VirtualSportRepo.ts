...
import {
  VirtualSportNode,
  VirtualSportChampGame,
  VirtualSportChampGameDetail,
} from '../types/VirtualSport';
  ...
  static async getChampions(): Promise<VirtualSportNode[]> {
    try {
      const { data } = await this.instance.get<{
        ...
        'GetVirtualSports?culture=pt-BR&timezoneOffset=180&integration=br4bet&deviceType=1&numFormat=en-GB&countryCode=BR',
      );
  ...
  static async getGameDetails(req: {
    ...
    try {
      const { data } = await this.instance.get<VirtualSportChampGameDetail>(
        `GetVirtualEventDetails?culture=pt-BR&timezoneOffset=180&integration=br4bet&deviceType=1&numFormat=en-GB&countryCode=BR&eventId=${req.eventId}&id=${req.id}&dbId=${req.dbId}`,
      );
